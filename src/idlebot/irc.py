...
class LineBuffer:
    ...
    def append(self, text):
        ...
        self.logger.debug(
            ": added %d bytes to socket buffer: total %d bytes",
            len(text),
            len(self._buffer),
        )
...
class SocketLineBuffer(LineBuffer):
    ...
    def __iadd__(self, data):
        ...
        try:
            ...
            self.logger.warning("Could not decode buffer data.")
            text = None
...
class Client:
    ...
    def _dispatcher(self, msg):
        ...
        if msg.startswith(":"):
            ...
            self.logger.warning("Unknown message -- %s", msg)
